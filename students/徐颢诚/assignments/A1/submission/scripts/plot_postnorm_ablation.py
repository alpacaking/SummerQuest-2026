"""绘制 TinyStories pre-norm 与 post-norm validation-loss 曲线。"""
...
if ROOT.name == "submission":
    ROOT = ROOT.parent
DEFAULT_PRENORM = ROOT / "logs" / "ts_baseline" / "2026-07-15" / "08-16-44" / "metrics.jsonl"
DEFAULT_OUTPUT = ROOT / "assets" / "tinystories_prenorm_vs_postnorm.svg"
...
def newest_postnorm_metrics() -> Path:
    candidates = list((ROOT / "logs" / "ablation_postnorm").glob("*/*/metrics.jsonl"))
    if not candidates:
        raise FileNotFoundError("没有找到 post-norm 训练日志")
    return max(candidates, key=lambda path: path.stat().st_mtime)


def write_svg(prenorm_path: Path, postnorm_path: Path, output_path: Path) -> None:
    runs = [
        ("Pre-norm baseline", read_points(prenorm_path), "#2563eb"),
        ("Post-norm", read_points(postnorm_path), "#dc2626"),
    ]
    if any(not points for _, points, _ in runs):
        raise ValueError("每个 run 都必须包含 validation-loss 点")
    ...
    svg = [
        ...
        f'<text x="{width / 2}" y="32" text-anchor="middle" font-size="20" font-weight="bold">Pre-norm vs. post-norm on TinyStories</text>',
...
def main() -> None:
    ...
    parser.add_argument("--prenorm", type=Path, default=DEFAULT_PRENORM)
    parser.add_argument("--postnorm", type=Path, default=None)
    parser.add_argument("--output", type=Path, default=DEFAULT_OUTPUT)
    args = parser.parse_args()
    postnorm_path = args.postnorm or newest_postnorm_metrics()
    write_svg(args.prenorm, postnorm_path, args.output)
    print(f"post-norm 日志：{postnorm_path}")
